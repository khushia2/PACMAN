...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	wdata[1] = val;
	alt_u8 rdata[1];
	int ret_val = alt_avalon_spi_command(SPI0_BASE, 0, 2, wdata, 0, NULL, 0); //1, rdata
	if (ret_val < 0) {
		printf("Error MAXreg_wr");
	}
}
//multiple-byte write
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	alt_u8 rdata[1];
	int ret_val = alt_avalon_spi_command(SPI0_BASE, 0, nbytes+1, wdata, 0, NULL, 0); //1, rdata
	if (ret_val < 0) {
		printf("Error MAXbytes_wr");
	}
	return (data+nbytes);
}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	wdata[0] = reg;
	alt_u8 rdata[1];
	alt_avalon_spi_command(SPI0_BASE, 0, 1, wdata, 1, rdata, 0);
	return *rdata;
}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	wdata[0] = reg;
	// alt_u8 rdata[nbytes];
	int ret_val = alt_avalon_spi_command(SPI0_BASE, 0, 1, wdata, nbytes, data, 0);
	if (ret_val < 0) {
		printf("Error MAXbytes_rd");
	}
	return (data + nbytes);
}
